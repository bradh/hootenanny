...
(reference layer). This scenario is commonly referred to as "Cut and Replace". To simply remove data without replacing it with anything, a
a single input dataset can be passed to this command. 

For replacement, the command can be configured to completely replace all reference data within a specified bounds or to only replace reference 
data in areas where the reference and secondary data overlap, so as to fill in gaps in the reference data. Additional filtering may be applied 
to the secondary input to control which types of features from it are used for replacement. Likewise, additional filtering may be applied to 
the reference input to prevent certain types of features from being replaced.
...
* +dataToCutAndOrReplace+        - Map input with features to replace; may be any supported input format that also supports bounded reading 
                                   (e.g. OSM file). See the "Input Data and "Bounds Handling" sections for more detail.
* +replacementData+              - Optional map input with the features to be used for replacement; If ommitted, data will be removed from the
                                   first input dataset and not replaced with anything. This may be any supported input format that supports 
...
* +--full-replacement+           - Determines how much of the reference data is replaced. With this option specified, all reference features 
                                   are replaced. With this option omitted, only reference features that overlap with secondary features are 
...
--------------------------------------
changeset-derive-replacement (dataToCutAndOrReplace) [replacementData] (bounds) (output) [osmApiDatabaseUrl] [--full-replacement] [--strict-bounds] [--geometry-filters] [--replacement-filters] [--chain-replacement-filters] [--replacement-filter-options] [--disable-way-snapping] [--disable-conflation] [--disable-oob-way-handling] [--stats filename] [--write-bounds]
--------------------------------------
...
The high level workflow for the command with the cut and replace scenario:

* Load the reference map (data you are replacing) at the specified bounds
* Filter the reference map based on geometry and/or other filters (optional)
* Load the secondary map (data you are using as replacement) at the specified bounds
...
The high level workflow for the command with cut only scenario:

* Load the reference map (data you are removing) at the specified bounds
* Filter the reference map based on geometry and/or other filters (optional)
* Cut data out of the reference map for each feature geometry type being replaced. This only works with full replacement.
* Derive a difference changeset between the new map with the cut out data added and the original reference map without removed data
